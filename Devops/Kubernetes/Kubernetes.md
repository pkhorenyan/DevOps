...
Как вы знаете, в кластере Kubernetes данные хранятся на PersistentVolume(PV). Ссылка на PersistentVolumeClaim (PVC) в манифесте Pod'а укажет, какой именно PV хранит данные. С Deployment есть нюанс, потому что манифесты всех Pod'ов одинаковые → все Pod'ы в Deployment могут ссылаться на один и тот же PVC, который связан с одним PV → во всех Pod'ах Deployment'а подключён один и тот же PV.

В StatefulSet проблема решена так: в описании манифеста Pod'а есть специальный раздел PVC-template, в котором описывается темплейт для PVC, и для каждого Pod'а будет создан свой PVC (например, data-test-1), который будет связан со своим уникальным PV. И в каждом Pod'е получим свой собственный том для хранения данных.

**Stateful applications**:

Базы данных (MongoDB, PostgreSQLm итд).
Разворачиваются через StatefulSet

**Stateless applications:**

Приложения не зависящие от предыдущих данных и не сохраняющие данные (NodeJS).
Разворачиваются через Deployment
