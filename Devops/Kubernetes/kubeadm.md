...
Конфигурируем MetalLB манифест для kubeadm:
...
spec:
  addresses:
  - 192.168.56.200-192.168.56.250 # Твой диапазон
---
apiVersion: metallb.io/v1beta1
...
Твой Service контроллера имеет тип NodePort, а не LoadBalancer.
Это значит, что MetalLB не управляет этим сервисом, и EXTERNAL-IP у него <none>

Тебе нужно сменить тип сервиса контроллера с NodePort на LoadBalancer.
Тогда MetalLB проснется и даст ему адрес.

```shell
kubectl --kubeconfig ~/.kube/kubeconfig-cluster patch svc ingress-nginx-controller -n ingress-nginx -p '{"spec": {"type": "LoadBalancer"}}'
```
Что произойдет после патча
...
В колонке EXTERNAL-IP у контроллера должен появиться адрес из твоего пула (например, 192.168.56.200).
  
Добавь запись в /etc/hosts на своей машине:

```
192.168.56.200  backend-frontend.local
```
 

Проверь через curl:

Должен ответить Nginx (Frontend)

```
curl -I http://backend-frontend.local/
```
 

Должен ответить Multitool (Backend)
``` 
curl -I http://backend-frontend.local/api
```